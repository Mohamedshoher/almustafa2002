...
/**
 * دالة مساعدة لإزالة أي قيم undefined من الكائن بعمق (Recursive)
 * لأن Firestore يرفض تخزين الـ undefined
 */
...
// مزامنة العملاء مع Firebase (بدون صور ومع تنظيف البيانات)
export const syncCustomerToCloud = async (customer: Customer) => {
  try {
    ...
    // 2. تنظيف الكائن بالكامل من أي قيم undefined قبل الإرسال
    const finalData = cleanObject(cloudData);
    
    await setDoc(doc(db, "customers", customer.id), finalData);
  } catch (error) {
